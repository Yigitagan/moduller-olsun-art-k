...
exports.run = function(client, message, args) {
 ...
 let yas = args[1]
  ...
  if(!isim) return message.channel.sendMessage('Seni Kayıt Etmem İçin Bir İsim Girmelisin: `!kayıt Mahmut `');
  if(!yas) return message.channel.sendMessage('Seni Kayıt Etmem İçin Bir Yaş Girmelisin: `!kayıt Mahmut 23`');
...
message.member.addRole(rol);
  ...
  client.channels.get(kanal3).send(`:scroll: ${user} Adlı Kullanıcı Kayıt Işlemini Başarıyla Tamamladı. <a:streaming:647108280790941717>`)
  ...
  user.setNickname(`${isim} | ${yas}`)
  message.channel.send(`:scroll: ${user} Kaydınız Oluşturuldu!`);
  client.channels.get(kanal3).send(`:scroll: ${user} Adlı Kullanıcı Kayıt Işlemini Başarıyla Tamamladı. <a:streaming:580081092468670464>`) 
   }else if(!kanal3) {
